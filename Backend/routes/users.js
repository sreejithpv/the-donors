...
import {User, validate, validateUsername, validateEmail, validatePassword, validatePage} from './../models/user';
...
router.post('/', async ({body}, res) => {
    ...
    user = new User(
        _.pick(body, ['fName', 'lName', 'email', 'username', 'password'])
    );
...
router.post('/page', async ({body}, res, next) => {

    const {error} = validatePage(body);
    if(error) return res.status(400).send(error.details[0].message);
...
router.post('/username/unique', async ({body}, res, next) => {

    const {error} = validateUsername(_.pick(body, ['username']));
    if(error) return res.status(400).send(error.details[0].message);
...
router.post('/email/unique', async ({body}, res, next) => {

    const {error} = validateEmail(_.pick(body, ['email']));
    if(error) return res.status(400).send(error.details[0].message);
...
// reset password
router.put('/reset', async ({body}, res, next) => {
